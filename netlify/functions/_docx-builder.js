...
function buildDocument(f, dealType) {
  ...
  const amt      = f.settlement_amount ? '$' + f.settlement_amount : '[SETTLEMENT AMOUNT]';
  ...
  const body = [
    ...
    numberedBold(3, 'CONFIDENTIALITY.', 'The Parties agree to keep the terms and existence of this Agreement strictly confidential and shall not disclose the terms to any third party without the prior written consent of the other Party, except as required by law, court order, or to their respective attorneys, accountants, or tax advisors.'),
    ...
    numberedBold(5, 'COUNTERPARTS / ELECTRONIC SIGNATURES.', 'This Agreement may be executed in two or more counterparts, each of which shall be deemed an original. Electronic and facsimile signatures shall be deemed original for all purposes.'),
...
function rescissionTerms(vehicle, miles, down, downWords, purchDate) {
  return [
    ...
    numberedBold(2, 'CANCELLATION OF RETAIL INSTALLMENT SALES CONTRACT.', `Upon receipt of the Vehicle, Dealer shall immediately cancel and rescind the Retail Installment Sales Contract dated ${purchDate}, and all associated financing obligations. Dealer shall notify all lienholders and finance companies within five (5) business days of Vehicle return.`),
    numberedBold(3, 'REFUND OF DOWN PAYMENT.', `Within five (5) calendar days of Dealer's receipt of the returned Vehicle, Dealer shall refund to Client the full down payment of ${down} (${downWords}). Payment shall be made payable to Client and Auto Legal Group, LLP, as directed in writing by ALG.`),
    ...
    numberedBold(5, 'CREDIT REPORTING.', 'Dealer shall ensure that no adverse or negative credit reporting arises from this transaction. Dealer shall remove any adverse credit entries within thirty (30) calendar days of execution of this Agreement.'),
    numberedBold(6, 'COOPERATION.', 'The Parties shall cooperate and execute all further documents necessary to carry out the rescission, including vehicle title transfer, lien releases, and DMV filings.'),
  ];
}
